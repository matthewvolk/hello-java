...
public class Monster {
	...
	// You should have as few public fields as possible
	...
	public void setHealth(int amountToDecreaseHealth) {
		health = health - amountToDecreaseHealth;
		if (health < 0) {
			alive = false;
		}
	}
	
	public void setHealth(double amountToDecreaseHealth) {
		int intAmountToDecreaseHealth = (int) amountToDecreaseHealth; 
		health = health - intAmountToDecreaseHealth;
		if (health < 0) {
			alive = false;
		}
	}
	...
	public void moveMonster(Monster[] monster, int arrayItemIndex) {
		...
		while(isSpaceOpen) {
			...
			// Erase the monster's current position by replacing it with a star
			...
			// When moving a monster, first thing we need to think is "is the target location to move to occupied?"
	...
	/*
	 * Class Constructor
	 * Code that is invoked when class is instantiated with 'new' operator
	 * 
	 */
	public Monster(int health, int attack, int movement, String name) {
		this.health = health; // 'this' is a reference to the actual object's variables
		this.attack = attack;
		...
		// Using a do loop because we don't want any kind of condition to prevent the loop from happening at least once
		do {
